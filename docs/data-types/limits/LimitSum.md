...
!!! info "Назначение Pydantic"
    Тип модели: **response/data**. Данные проверяются вызовом `LimitSum.model_validate(payload)`. При несовпадении типов или отсутствии обязательного поля Pydantic формирует `ValidationError`.
