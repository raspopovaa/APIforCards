...
Документация публикуется по линиям совместимости `major.minor`. Для SDK 2.3.0
используется каталог `2.2`, а alias `latest` перенаправляет на текущую линию.

Примеры URL:

- `/APIforCards/2.2/` — документация линии 2.2;
...
1. Обновите версию SDK в `pyproject.toml`.
2. Обновите примеры и сведения о совместимости.
3. Запустите генераторы и release-gate.
