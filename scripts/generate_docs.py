...
def render_catalog(grouped: dict[str, list[Any]], metadata: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Общий формат ответа",
            "",
            "Большинство методов возвращают типизированную модель SDK. Модель содержит "
            "данные ответа API; ошибки преобразуются в исключения SDK.",
            "",
            "Подробные структуры моделей приведены в разделе "
...
def render_model_index(models: list[type[BaseModel]]) -> str:
    lines = ["# Типы данных", "", "Типизированные модели запросов и ответов SDK.", ""]
