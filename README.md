...
`Decimal` и сериализуют сумму строкой согласно спецификации v1.1.60.

Все публичные параметры сервисов передаются только по имени. JSON-методы
возвращают полный типизированный envelope `status/data/timestamp`; бинарные
отчёты можно получить в память или потоково сохранить методом
`client.reports.download_report_file_to()`.
