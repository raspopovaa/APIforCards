...
- явный и стабильный выбор договора при авторизации и re-auth;
- безопасные retry для JSON и binary download;
...
Единственный доступный договор SDK выбирает автоматически. При нескольких
договорах передайте `contract_id` или `contract_number`; иначе будет вызван
...
- **SDK асинхронный.** Методы вызываются через `await`, а клиент рекомендуется
  использовать как асинхронный контекстный менеджер. Выход из контекста закрывает
  локальные ресурсы, но серверный `logoff` нужно явно вызывать в `finally`.
- **Внешний transport остаётся во владении вызывающего кода.** `APIClient` закрывает
  только transport, который создал самостоятельно.
- **DEMO-стенд ограничивает частоту запросов.** Для него рекомендуется начинать с
  `API_REQUESTS_PER_SECOND=2`.
- **Договор не выбирается по порядку списка.** При нескольких договорах требуется
  явный ID или номер; re-auth повторно проверяет тот же договор.
...
- **JSON и binary download используют общую retry policy.** Повтор допускается
  только для безопасных и идемпотентных операций.
